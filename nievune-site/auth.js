...
(() => {
  ...
  function configureSocialAuthUI() {
    const enabled = config.socialAuth || {};
    document.querySelectorAll('[data-social-auth]').forEach((group) => {
      let visibleButtons = 0;
      group.querySelectorAll('[data-social-provider]').forEach((button) => {
        const isEnabled = enabled[button.dataset.socialProvider] === true;
        button.hidden = !isEnabled;
        if (isEnabled) visibleButtons += 1;
      });
      group.hidden = visibleButtons === 0;
    });
  }

  async function signInWithSocialProvider(event) {
    const button = event.currentTarget;
    const provider = button.dataset.socialProvider;
    const view = button.closest('[data-auth-view]')?.dataset.authView;
    const feedbackId = view === 'signup' ? 'signUpFeedback' : 'signInFeedback';
    if (!ensureConfigured(feedbackId)) return;

    const providerName = provider === 'apple' ? 'Apple' : 'Google';
    setFeedback(feedbackId, `Opening ${providerName}…`, 'info');
  ...
  function bindUI() {
    ...
    modal?.querySelectorAll('[data-social-provider]').forEach((button) => {
      button.addEventListener('click', signInWithSocialProvider);
    });
